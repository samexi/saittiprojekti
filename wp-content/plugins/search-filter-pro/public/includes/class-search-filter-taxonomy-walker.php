	...
	function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 )
	{
		...
		else if(($this->type=="checkbox")||($this->type=="radio"))
		{
			...
			//{
				...
				$link .= "<label><input type='".$this->type."' name='".$name."[]' value='".$cat_slug."'".$checked.$disabled." data-sf-cr='".SF_TAX_PRE.$cat_id."' data-sf-hide-empty='".intval($hide_empty)."' /> ".$cat_name;
				...
				if ( !empty($show_count) )
				{
					...
					$link .= ' <span class="'.SF_CLASS_PRE.'count">(' . $term_count . ')</span>';
					//$link .= ' <span class="'.SF_CLASS_PRE.'count">(' . intval($category->count) . ')</span>';
				}
