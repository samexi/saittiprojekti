					...
					if($key == "category")
					{
						if(isset($val['ids']))
						{
							if($val['ids']!="")
							{
								...
								{
									$query->set('category__in', array_map("intval" , explode(",", $val['ids'])));
								}
								else
								{
									$query->set('category__not_in', array_map("intval" , explode(",", $val['ids'])));
								}
							}
						}
					}
					else if($key=="post_tag")
					{
						if(isset($val['ids']))
						{
							if($val['ids']!="")
							{
								if($val["include_exclude"]=="include")
								{
									$query->set('tag__in', array_map("intval" , explode(",", $val['ids'])));
								}
								else
								{
									$query->set('tag__not_in', array_map("intval" , explode(",", $val['ids'])));
								}
							}
						}
					}
						...
						if(isset($val['ids']))
						{
							if($val['ids']!="")
							{
								...
								$this->tax_query[] = array(
									'taxonomy' => $key,
									'field'    => 'id',
									'terms'    => array_map("intval" , explode(",", $val['ids'])),
									'operator' => $operator
								);
							}
						}	
		...
		if($sf_form_data->settings('settings_post_meta')!="")
		{
			...
			if(is_array($sf_form_data->settings('settings_post_meta')))
			{
				...
				{					
					...
					else if($post_meta['meta_type']=="DATE")
					{
